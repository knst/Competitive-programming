...
int main()
{
    ...
    int m;
    cin >> n >> m;
    vector<pair<Point,int>> vn(n);
    vector<pair<Point,int>> vm(m);
    for (int i = 0; i < n; ++i) {
        cin >> vn[i].first.first >> vn[i].first.second >> vn[i].second;
    }

    for (int j = 0; j < m; ++j) {
        cin >> vm[j].first.first >> vm[j].first.second >> vm[j].second;
    }
    int maximal = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (vn[i].first.first > vm[j].first.first && vn[i].first.first < vm[j].first.first + vm[j].second &&
                vm[j].first.second > vn[i].first.second && vm[j].first.second < vn[i].first.second + vn[i].second) {

                int len1 = vn[i].first.first - vm[j].first.first;
                int len2 = vm[j].first.first + vm[j].second - vn[i].first.first;
                int len3 = vm[j].first.second - vn[i].first.second;
                int len4 = vn[i].first.second + vn[i].second - vm[j].first.second;
